...
keyword_pattern = re.compile(r'defi|int|is|isnot|show|program_out')
identifier_pattern = re.compile(r'[a-zA-Z_][a-zA-Z0-9_]*')
constant_pattern = re.compile(r'\d+(\.\d+)?')
operator_pattern = re.compile(r'==|\?')
delimiter_pattern = re.compile(r'[():{}]')
string_literal_pattern = re.compile(r'"[^"]*"')  # Updated regex for string literals
...
def lexical_analysis(source_code):
    ...
    while i < len(source_code):
        ...
        if char.isalpha() or char == '_':
            ...
            token = ""
            while i < len(source_code) and (char.isdigit() or char == '-' or char == '.'):
                token += char
                i += 1
                if i < len(source_code):
                    char = source_code[i]
            ...
            if match:
                token = match.group(0)
                add_token_to_symbol_table(token)  # Add the string literal to the symbol table
                add_token_to_fip(5)  # Use code 5 for string literals
                i += len(token)
